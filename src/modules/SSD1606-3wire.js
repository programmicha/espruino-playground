 ...
 * All display data is based on specification version 'A/0(For SLC 505a FH E21002-DL Ver02 TFT' from 22.10.2012.
 ...
 * Max screenbytes are for 172*72 / 4 = 3096 bytes (4 Pixels per byte).
 */
var C  = {
  GDE021A1: {
    bpp               : 2,
 ...
 * <li>Set spiMode to either 0 for using 4 lines or 3 for using 3 lines.</li>
 * <li>Don't forget to set the bs1Pin when using spiMode 0 aka 4 lines.</li>
 * </ul>
 * @constructor
...
/**
 * Initialize display.
 * If set it uses the provided bs1Pin to configure the SPI mode between using 3 or 4 lines.
...
SSD1606.prototype.init = function(callback, options) {
  if(this.bs1Pin) {
    if((this.spiMode != undefined) && (this.spiMode === 0)) {
      digitalWrite(this.bs1Pin, LOW);
    } else {
      digitalWrite(this.bs1Pin, HIGH);
    }
  }
  this.scd(0x10, 0x00);
  this.scd(0x11, 0x03);
 ...
 * Uses the dcPin if spimode is set to 4-lines.
 */
...
SSD1606.prototype.sd = function(data) {
  if(this.spiMode === 0) {
    ...
    //} else {
      //digitalWrite(this.cs1Pin, LOW);
      //digitalWrite(this.cs1Pin, HIGH);
      // add 'it is data' to the data by setting an additional 9. left bit
      //data |= (1 << 8);
      //this.spi.write(data, this.cs1Pin);
    //}
  }
...
/**
 * Send display data the controller.
 * @param {command} - the command
 * @param {data} - the data
 */
SSD1606.prototype.sdd = function(command, data) {
  if( Object.prototype.toString.call(data) === '[object Array]' ) {
    // set the first data byte to contain 9 bit with command
    data[0] |= (1 << 8);
    //data.forEach(function(part, index, theArray) {
      //theArray[index] |= (1 << 8);
    //});
 ...
 * just write the bytes. To leave the write RAM mode a 'NOP' command is needed.
...
SSD1606.prototype.csb = function(callback, clearScreenColor) {
  ...
  for (i = 0; i < this.display.maxScreenBytes; i++) {
    this.scd(clearScreenColor);
  }
  this.sc(0xFF);
  return setTimeout(callback, this.csbTimeOut); //busPin does not work here, seems to be the display driver encapsulates the behaviour
};
/**
...
SSD1606.prototype.refreshScreen = function(callback) {
  ...
  this.sc(0x20);
  // following is called 'closeBump' in specification
  this.scd(0x22, 0x03);
  this.sc(0x20);
...
/**
 * Create the Graphics object with Graphics.createArrayBuffer(...).
